@extends('auth.app')
@section('title', 'データの移行 | PIST6オフィシャルサイト')
@section('content')
    <div class="l-content">
                  ...
                  <section class="rg-regist _regist">
                    <div class="rg-regist__inner">
                      <header class="rg-regist__header">
                        <h2 class="rg-regist__header-txt al">SMS送信</h2>
                        <p class="mt30 fs14">ご登録ありがとうございます。<br>先ほど、登録認証メッセージをお送りしました。<br>メッセージに従い、移行手続きを進めて下さい。</p>
                      </header>
                      <form action={{ route('transfer.register') }} method='post' class="rg-regist__body" id='veriftySMS'>
                        @csrf
                        <input type="hidden" name="phone_number" value="{{$phone_number??old('phone_number')}}">
                        <input type="hidden" name="old_user_id" value="{{$old_user_id??old('old_user_id')}}">
                        <div class="rg-regist__info">
                          ...
                          <div class="rg-regist__info-item">
                            <p class="rg-regist__label">確認コード<span class="rg-regist__label-required">必須</span></p>
                            <div class="rg-regist__inputs">
                              <div class="rg-regist__inputs-item">
                                <input class="c-input-text" type="number" name="pin_code" oninput="javascript: this.value = this.value.slice(0, 6);" id='pin_code'>
                              </div>
                            </div>
                          </div>
                        </div>
                        <div class="rg-regist__submit">
                          <div class="rg-regist__submit-inner">
                            <button class="c-btn-rect brid30" type="button" id="digitCheck"><span class="c-btn-rect__txt">データ移行へ</span></button>
                          </div>
                        </div>
                        <div class="rg-regist__submit">
                          <div class="rg-regist__submit-inner">
                            <p class="fs14">コードが届かない場合は再送、 または電話番号を確認後に再度入力してください</p>
                            <button form="regenerate" class="c-btn-rect brid30 w50" type="submit"><span class="c-btn-rect__txt">コード再送</span></button>
                          </div>
                        </div>
                    </form>
                    <form id="regenerate" action="{{ route('transfer.sms.cert') }}" method="POST">
                      @csrf
                      <input type="hidden" name="phone_number" value="{{$phone_number??old('phone_number')}}">
