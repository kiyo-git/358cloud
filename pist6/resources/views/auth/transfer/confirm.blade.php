@extends('auth.app')
@section('title', 'データ移行ユーザー会員登録 | PIST6オフィシャルサイト')
@section('content')
    <div class="l-content">
            ...
            <header class="l-regist__header">
              <div class="rg-header">
                <div class="rg-header__inner">
                  <h2 class="rg-header__ttl"><span class="rg-header__ttl-purpose">データ移行ユーザー会員登録<span class="rg-header__ttl-action">お客様情報の入力</span></span></h2>
                  <p class="rg-header__lead">入力内容をご確認ください。よろしければ「登録する」ボタンを押してください。</p>
                </div>
              </div>
            </header>
                ...
                <div class="rg-body__regist">
                  <section class="rg-regist _regist">
                    <div class="rg-regist__inner">
                      ...
                      <form action="{{ route('transfer.complete') }}" method="POST"   class="rg-regist__body">
                        @csrf
                        <div class="rg-regist__confirm">
                          <div class="rg-regist__confirm-item">
                            <dl class="rg-regist__define">
                              <dt class="rg-regist__define-head">名前</dt>
                              <dd class="rg-regist__define-body">{{$insert_items->family_name}} {{$insert_items->given_name}}</dd>
                            </dl>
                          </div>
                          <div class="rg-regist__confirm-item">
                            <dl class="rg-regist__define">
                              <dt class="rg-regist__define-head">フリガナ</dt>
                              <dd class="rg-regist__define-body">{{$insert_items->family_name_kana}} {{$insert_items->given_name_kana}}</dd>
                            </dl>
                          </div>
                          <div class="rg-regist__confirm-item">
                            <dl class="rg-regist__define">
                              <dt class="rg-regist__define-head">生年月日</dt>
                              <dd class="rg-regist__define-body">{{$insert_items->birth}}</dd>
                            </dl>
                          </div>
                          <div class="rg-regist__confirm-item">
                            <dl class="rg-regist__define">
                              <dt class="rg-regist__define-head">電話番号</dt>
                              <dd class="rg-regist__define-body">{{ $insert_items->phone_number }}</dd>
                            </dl>
                          </div>
                          <div class="rg-regist__confirm-item">
                            <dl class="rg-regist__define">
                              <dt class="rg-regist__define-head">郵便番号</dt>
                              <dd class="rg-regist__define-body">{{ $insert_items->zip_code }}</dd>
                            </dl>
                          </div>
                          <div class="rg-regist__confirm-item">
                            <dl class="rg-regist__define">
                              <dt class="rg-regist__define-head">住所[都道府県]</dt>
                              <dd class="rg-regist__define-body">{{ $insert_items->prefecture }}</dd>
                            </dl>
                          </div>
                          <div class="rg-regist__confirm-item">
                            <dl class="rg-regist__define">
                              <dt class="rg-regist__define-head">住所[市区町村]</dt>
                              <dd class="rg-regist__define-body">{{ $insert_items->city }}</dd>
                            </dl>
                          </div>
                          <div class="rg-regist__confirm-item">
                            <dl class="rg-regist__define">
                              <dt class="rg-regist__define-head">住所[番地]</dt>
                              <dd class="rg-regist__define-body">{{ $insert_items->block }}</dd>
                            </dl>
                          </div>
                          <div class="rg-regist__confirm-item">
                            <dl class="rg-regist__define">
                              <dt class="rg-regist__define-head">住所[マンション・号室]</dt>
                              <dd class="rg-regist__define-body">{{ $insert_items->buliding }} </dd>
                            </dl>
                          </div>
                          <div class="rg-regist__confirm-item">
                            <dl class="rg-regist__define">
                              <dt class="rg-regist__define-head">メールアドレス</dt>
                              <dd class="rg-regist__define-body">{{ $insert_items->email }}  </dd>
                            </dl>
                          </div>
                          <div class="rg-regist__confirm-item">
                            <dl class="rg-regist__define">
                              <dt class="rg-regist__define-head">メルマガ配信</dt>
                              <dd class="rg-regist__define-body">{{ ($insert_items->mailmagazine=0)?'受け取らない':'受け取る' }}</dd> {{--ccofigに持たせたい--}}
                            </dl>
                          </div>
                        </div>
                        {{-- <input type="hidden" name="user_check" value="{{ $insert_items->ng_user_check}}">
                        {{-- <input type="hidden" name="qr_user_id" value="{{ $old_user->qr_user_id}}"> --}}
                        <div class="rg-regist__submit">
                          <div class="rg-regist__submit-inner">
                            <button class="c-btn-rect" type="submit"><span class="c-btn-rect__txt">登録する</span></button>
                          </div>
                        </div>
                        <div class="rg-regist__submit kmt00">
                          <div class="rg-regist__submit-inner">
                            <button class="c-btn-rect gray" type="button" onclick="location.href='{{ route('transfer.register.view') }}' "><span class="c-btn-rect__txt">戻る</span></button>
                          </div>
                        </div>
                    </form>
                    </div>
