...
return new class extends Migration
{
    ...
    public function up()
    {
        Schema::create('temp_users', function (Blueprint $table) {
            $table->comment('ユーザー登録情報の一時的な格納テーブル');

            $table->id()->comment('id');
            $table->foreignId('user_certification_id')->comment('一般ユーザー認証情報ID（user_certifications.id）');
            $table->string('email')->unique()->comment('メールアドレス');
            $table->string('password')->comment('パスワード');
            $table->rememberToken()->nullable()->comment('RememberMeトークン');
            ...
            $table->smallInteger('mailmagazine_flg')->default(0)->comment('メルマガ登録フラグ');
            $table->smallInteger('ng_user_check')->default(0)->comment('NGユーザーチェック');
